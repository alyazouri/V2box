function FindProxyForURL(url, host) {
  ...
  // ===== دالة بناء سلسلة بروكسي للألعاب (UDP سريع) =====
  function buildGameProxy() {
    var chain = [];
    for (var i = 0; i < gamePorts.length; i++) {
      chain.push("SOCKS5 " + proxyIP + ":" + gamePorts[i]);
    }
    return chain.join("; "); // Failover تلقائي
  }

  // ===== دالة بناء سلسلة بروكسي للإنترنت العادي =====
  ...
  // ===== HTTPS / HTTP =====
  var isHttps = url.substring(0, 5).toLowerCase() === "https";
  ...
  // ===== باقي الإنترنت العادي يمر عبر البروكسي على بورت خفيف وطيف =====
